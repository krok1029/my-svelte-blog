...
>
		...
		<div class="relative flex h-16 items-center justify-between">
			...
			<DropdownMenu>
				<DropdownMenuTrigger asChild>
					<Button variant="ghost" size="icon" class="rounded-full" id="user-menu-button">
						<User2 class="h-6 w-6" />
				...
				<DropdownMenuContent align="end" class="w-48">
					{#if $authUser}
						<DropdownMenuItem onclick={() => goto('/')}>Your Profile</DropdownMenuItem>
						<DropdownMenuItem onclick={() => goto('/admin')}>Admin</DropdownMenuItem>
						<DropdownMenuSeparator />
						<DropdownMenuItem onclick={handleLogout}>Logout</DropdownMenuItem>
					{:else}
						<DropdownMenuItem onclick={() => goto('/login')}>Login</DropdownMenuItem>
						<DropdownMenuItem onclick={() => goto('/register')}>Register</DropdownMenuItem>
					{/if}
				</DropdownMenuContent>
			</DropdownMenu>
		</div>
	...
	<CollapsibleContent
		class="border-t border-gray-200 bg-white sm:hidden dark:border-gray-700 dark:bg-gray-900"
	>
		<div class="space-y-1 px-2 pt-2 pb-3">
			{#each navbarList as navbar}
				<a
					href={navbar.link}
					class="block rounded-md px-3 py-2 text-base font-medium transition-colors {checkCurrentUrl(
						page,
						navbar.link
					)}"
				>
