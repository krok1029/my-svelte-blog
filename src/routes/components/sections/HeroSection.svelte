<script lang="ts">
	...
	const getSkillColor = (skill: string) => {
		...
		return colors[skill] || 'bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200';
	};
</script>
	...
	<div class="container mx-auto px-4 py-20">
		<div class="mx-auto grid max-w-6xl grid-cols-1 items-start gap-10 md:grid-cols-2">
			...
			<div>
				...
				<h1
					class="mb-2 text-4xl leading-tight font-extrabold text-gray-900 sm:text-5xl dark:text-gray-100"
				>
					<span
				...
				<p class="mb-6 text-lg leading-relaxed text-gray-700 dark:text-gray-300">
					目前以前端為主（<strong>4 年</strong>，Cyberbiz）， 也有
					<strong>.NET 後端 2 年</strong>（Kenmec）經驗。 在 Cyberbiz 擔任過
					<strong>Scrum Master</strong>， 關注產品交付節奏、品質與跨部門協作。
					我熱愛打造穩定、可擴充、好維護的使用者體驗， 同時也樂於嘗試並承擔更多後端與平台相關工作。
				</p>
				...
				<ul class="mb-8 flex flex-wrap gap-2">
					{#each tags as t}
						<li
							class="rounded-full border border-gray-300 px-3 py-1 text-xs text-gray-700 dark:border-gray-600 dark:text-gray-300"
						>
							{t}
						</li>
					{/each}
				...
				<div class="flex flex-wrap gap-3">
					<a
						href="#projects"
						class="inline-flex items-center justify-center rounded-xl bg-purple-600 px-5 py-3 text-sm font-medium text-white shadow hover:bg-purple-700 focus:ring-2 focus:ring-purple-500 focus:outline-none"
						aria-label="查看作品專案">查看作品</a
					>
					<a
						href="#contact"
						class="inline-flex items-center justify-center rounded-xl border border-gray-300 bg-white px-5 py-3 text-sm font-medium text-gray-900 hover:bg-gray-50 focus:ring-2 focus:ring-purple-500 focus:outline-none dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100 dark:hover:bg-gray-700"
						aria-label="與我聯絡">與我聯絡</a
					>
				</div>
			</div>
			...
			<div class="space-y-6">
				{#each experiences as exp}
					<div
						class="rounded-xl border border-gray-100 bg-white p-6 shadow-lg dark:border-gray-700 dark:bg-gray-800"
					>
						<div class="flex items-start gap-4">
							...
							<div class="flex-1">
								...
								<div
									class="mb-3 flex flex-wrap items-center gap-4 text-gray-600 dark:text-gray-400"
								>
				...
				<!-- 目標句 -->
				<div
					class="rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 p-4 text-sm text-white"
				>
					目前<strong>以前端為主</strong>，並持續投入<strong>後端 / 平台</strong
					>（Vite、架構遷移、Electron、Tauri）等領域的實務。
				</div>
			</div>
		</div>
	</div>
